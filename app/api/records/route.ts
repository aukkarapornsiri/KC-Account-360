...
const parseMetadata = (value: string) => { try { const parsed = JSON.parse(value); return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed as Record<string, unknown> : {}; } catch { return {}; } };
const MODULE_DEFAULT_STATUS: Record<string, string> = { GL: "Draft", AR: "Pending Approval", AP: "Pending Approval", CASH: "Unreconciled", TAX: "Preparing", INTEGRATION: "Queued", CLOSING: "Pending", BUDGET: "Active" };
const SALES_DOCUMENT_STATUS: Record<string, string> = { Quotation: "Draft", "Sales Order": "Pending Approval", "Deposit Receipt": "Pending Approval", "Delivery Note": "Draft", Invoice: "Pending Approval", "Billing Note": "Draft", Receipt: "Received", "Credit Note": "Pending Approval", "Debit Note": "Pending Approval" };
...
async function seedIfEmpty(email: string) {
  ...
  const settingSeed = [
    { key: "company_name", value: "KC Account 360", updatedBy: email, updatedAt: now },
    { key: "tax_id", value: "", updatedBy: email, updatedAt: now },
    { key: "approval_limit", value: "500000", updatedBy: email, updatedAt: now },
    { key: "current_period", value: PERIOD, updatedBy: email, updatedAt: now },
    { key: "locked_period", value: "", updatedBy: email, updatedAt: now },
  ];
  const masterSeed = [
    { id: crypto.randomUUID(), category: "COMPANY", code: "COMPANY-001", name: "KC Account 360", description: "กรุณากำหนดข้อมูลบริษัทก่อนเริ่มใช้งาน", status: "Active", metadata: JSON.stringify({ taxId: "" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "BRANCH", code: "BR-00000", name: "สำนักงานใหญ่", description: "กรุณากำหนดที่อยู่สาขา", status: "Active", metadata: JSON.stringify({ company: "COMPANY-001" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "110100", name: "เงินสดและรายการเทียบเท่าเงินสด", description: "สินทรัพย์หมุนเวียน", status: "Active", metadata: JSON.stringify({ type: "Asset" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "120100", name: "ลูกหนี้การค้า", description: "สินทรัพย์หมุนเวียน", status: "Active", metadata: JSON.stringify({ type: "Asset" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "210100", name: "เจ้าหนี้การค้า", description: "หนี้สินหมุนเวียน", status: "Active", metadata: JSON.stringify({ type: "Liability" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "310100", name: "ทุนจดทะเบียน", description: "ส่วนของผู้ถือหุ้น", status: "Active", metadata: JSON.stringify({ type: "Equity" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "410100", name: "รายได้จากการขายและบริการ", description: "รายได้", status: "Active", metadata: JSON.stringify({ type: "Revenue" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "ACCOUNT", code: "510100", name: "ต้นทุนขายและบริการ", description: "ต้นทุน", status: "Active", metadata: JSON.stringify({ type: "Expense" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "USER", code: "USR-FIN-ADMIN", name: email, description: "Finance Administrator", status: "Active", metadata: JSON.stringify({ role: "Admin", scope: "All branches" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "MAPPING", code: "MAP-CUTO-AR", name: "KC CuTo CRM → AR Invoice", description: "customer_id → counterparty, net_total → amount", status: "Active", metadata: JSON.stringify({ source: "KC CuTo CRM", target: "AR" }), createdBy: email, createdAt: now, updatedAt: now },
    { id: crypto.randomUUID(), category: "MAPPING", code: "MAP-TORY-AP", name: "KC Inventory → AP / Inventory Accounting", description: "vendor_code → counterparty, gr_value → amount", status: "Active", metadata: JSON.stringify({ source: "KC Inventory", target: "AP" }), createdBy: email, createdAt: now, updatedAt: now },
...
export async function POST(request: Request) {
  ...
  if (body.action === "create") {
    ...
    const required = ["module", "recordType", "documentNo", "description"];
    ...
    if (!/^[A-Z0-9][A-Z0-9._/-]{2,39}$/i.test(documentNo)) return jsonError("เลขที่เอกสารต้องมี 3–40 ตัวอักษร และใช้ตัวอักษร ตัวเลข - _ / . เท่านั้น", 422);
    ...
    let amount = Number(body.amount ?? 0); let taxAmount = Number(body.taxAmount ?? 0);
    let metadata = "{}";
    ...
    if (["AR", "AP"].includes(moduleKey)) {
      ...
      if (moduleKey === "AR") {
        ...
        const [customer] = await db.select({ id: masterData.id }).from(masterData).where(and(eq(masterData.category, "CUSTOMER"), eq(masterData.name, counterparty), eq(masterData.status, "Active"))).limit(1);
      ...
      const lineItems = rawItems.slice(0, 10).map((value) => {
        const item = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {};
        return {
          code: String(item.code || "").trim().slice(0, 80), description: String(item.description || "").trim().slice(0, 200),
          unit: String(item.unit || "รายการ").trim().slice(0, 40), quantity: Number(item.quantity || 0),
          unitPrice: Number(item.unitPrice || 0), discount: Number(item.discount || 0),
        };
      ...
      const taxRate = Number(rawMetadata.taxRate || 0); const whtRate = Number(rawMetadata.whtRate || 0);
      ...
      const referenceDocumentNo = String(rawMetadata.referenceDocumentNo || rawMetadata.linkedDocumentNo || "").trim().slice(0, 40);
      ...
      const cleanText = (key: string, limit: number) => String(rawMetadata[key] || "").trim().slice(0, limit);
      ...
      if (vendorInvoiceNo) {
        const vendorInvoices = await db.select({ counterparty: financialRecords.counterparty, metadata: financialRecords.metadata }).from(financialRecords).where(and(eq(financialRecords.module, "AP"), eq(financialRecords.recordType, "Purchase Invoice")));
        if (vendorInvoices.some((item) => item.counterparty.trim().toLowerCase() === counterparty.toLowerCase() && String(parseMetadata(item.metadata).vendorInvoiceNo || "").trim().toLowerCase() === vendorInvoiceNo.toLowerCase())) return jsonError("เลข Invoice ของ Vendor นี้ถูกบันทึกแล้ว", 409);
      }
      ...
      metadata = JSON.stringify({
        documentCode: documentDefinition?.code, issueDate, referenceDocumentNo, linkedDocumentNo, paymentTerms, taxRate, whtRate, lineItems, vendorInvoiceNo, documentTiming,
        counterpartyAddress: cleanText("counterpartyAddress", 500), counterpartyTaxId: cleanText("counterpartyTaxId", 20),
        contactName: cleanText("contactName", 120), projectName: cleanText("projectName", 200), preparedBy: cleanText("preparedBy", 120),
        notes: cleanText("notes", 1000), paymentInstructions: cleanText("paymentInstructions", 1000),
        subtotal: cents(amount), withholdingTax: cents(withholdingTax), total: cents(amount + taxAmount), netTotal: cents(amount + taxAmount - withholdingTax), affectsStock,
      });
    ...
    try {
      await db.insert(financialRecords).values({ id, module: moduleKey, recordType, documentNo, sourceSystem: String(body.sourceSystem || "KC Account"), counterparty: String(body.counterparty || "").slice(0, 200), description, amount: cents(amount), taxAmount: cents(taxAmount), currency: "THB", status, dueDate: body.dueDate ? String(body.dueDate) : null, period, metadata, createdBy: user.email, approver: null, postedAt: ["Receipt"].includes(recordType) ? now : null, createdAt: now, updatedAt: now });
    } catch { return jsonError("เลขที่เอกสารซ้ำ หรือข้อมูลไม่ถูกต้อง", 409); }
    await db.insert(auditLogs).values({ recordId: id, action: "CREATE", actorEmail: user.email, details: `Created ${recordType} ${documentNo}`, createdAt: now });
  ...
  if (body.action === "update_document") {
    ...
    const taxAmount = amount * taxRate / 100; const withholdingTax = amount * whtRate / 100;
    const referenceDocumentNo = String(rawMetadata.referenceDocumentNo || rawMetadata.linkedDocumentNo || "").trim().slice(0, 40);
    ...
    if (vendorInvoiceNo) {
      const vendorInvoices = await db.select({ id: financialRecords.id, counterparty: financialRecords.counterparty, metadata: financialRecords.metadata }).from(financialRecords).where(and(eq(financialRecords.module, "AP"), eq(financialRecords.recordType, "Purchase Invoice")));
      if (vendorInvoices.some((item) => item.id !== id && item.counterparty.trim().toLowerCase() === counterparty.toLowerCase() && String(parseMetadata(item.metadata).vendorInvoiceNo || "").trim().toLowerCase() === vendorInvoiceNo.toLowerCase())) return jsonError("เลข Invoice ของ Vendor นี้ถูกบันทึกแล้ว", 409);
    }
    ...
    const metadata = JSON.stringify({
      documentCode: definition.code, issueDate, referenceDocumentNo, linkedDocumentNo: referenceDocumentNo, paymentTerms, taxRate, whtRate, lineItems, vendorInvoiceNo, documentTiming,
      counterpartyAddress: cleanText("counterpartyAddress", 500), counterpartyTaxId: cleanText("counterpartyTaxId", 20),
      contactName: cleanText("contactName", 120), projectName: cleanText("projectName", 200), preparedBy: cleanText("preparedBy", 120),
      notes: cleanText("notes", 1000), paymentInstructions: cleanText("paymentInstructions", 1000),
      subtotal: cents(amount), withholdingTax: cents(withholdingTax), total: cents(amount + taxAmount), netTotal: cents(amount + taxAmount - withholdingTax), affectsStock,
    });
    await db.update(financialRecords).set({ counterparty: counterparty.slice(0, 200), description: lineItems.map((item) => item.description).join(", ").slice(0, 500), amount: cents(amount), taxAmount: cents(taxAmount), dueDate, metadata, updatedAt: now }).where(eq(financialRecords.id, id));
    await db.insert(auditLogs).values({ recordId: id, action: "UPDATE_DOCUMENT", actorEmail: user.email, details: `Updated ${record.recordType} ${record.documentNo}`, createdAt: now });
  ...
  if (body.action === "update_record") {
    ...
    if (record.module === "AR") {
      try { if (Array.isArray((JSON.parse(record.metadata) as { lineItems?: unknown }).lineItems)) return jsonError("เอกสารขายแบบมีรายการสินค้าไม่อนุญาตให้แก้ยอดรวมโดยตรง กรุณายกเลิกและสร้างเอกสารใหม่", 409); } catch {}
    }
    ...
    const description = String(body.description || "").trim();
    const counterparty = String(body.counterparty || "").trim();
    const amount = Number(body.amount ?? 0); const taxAmount = Number(body.taxAmount ?? 0);
    const dueDate = body.dueDate ? String(body.dueDate) : null;
    ...
    if (!Number.isFinite(amount) || amount < 0 || !Number.isFinite(taxAmount) || taxAmount < 0) return jsonError("จำนวนเงินหรือภาษีไม่ถูกต้อง", 422);
    if (dueDate && !/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) return jsonError("รูปแบบวันครบกำหนดไม่ถูกต้อง", 422);
    await db.update(financialRecords).set({ description, counterparty, amount: cents(amount), taxAmount: cents(taxAmount), dueDate, updatedAt: now }).where(eq(financialRecords.id, id));
    await db.insert(auditLogs).values({ recordId: id, action: "UPDATE", actorEmail: user.email, details: `Updated ${record.documentNo}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }
  ...
  if (body.action === "update_settings") {
    ...
    const entries = Object.entries(settingValues).filter(([key]) => allowed.has(key)).map(([key, value]) => [key, String(value ?? "").trim()] as const);
    ...
    for (const [key, value] of entries) await db.insert(settings).values({ key, value, updatedBy: user.email, updatedAt: now }).onConflictDoUpdate({ target: settings.key, set: { value, updatedBy: user.email, updatedAt: now } });
    await db.insert(auditLogs).values({ recordId: null, action: "UPDATE_SYSTEM_SETTINGS", actorEmail: user.email, details: `Updated ${entries.map(([key]) => key).join(", ")}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }
  ...
  if (body.action === "update_setting") {
    ...
    await db.insert(settings).values({ key, value: String(body.value ?? ""), updatedBy: user.email, updatedAt: now }).onConflictDoUpdate({ target: settings.key, set: { value: String(body.value ?? ""), updatedBy: user.email, updatedAt: now } });
    await db.insert(auditLogs).values({ recordId: null, action: "UPDATE_SETTING", actorEmail: user.email, details: `Updated ${key}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }
  ...
  if (body.action === "lock_period") {
    ...
    await db.insert(settings).values({ key: "locked_period", value: period, updatedBy: user.email, updatedAt: now }).onConflictDoUpdate({ target: settings.key, set: { value: period, updatedBy: user.email, updatedAt: now } });
    await db.insert(auditLogs).values({ recordId: null, action: "LOCK_PERIOD", actorEmail: user.email, details: `Locked accounting period ${period}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }

  if (body.action === "create_master") {
    const category = String(body.category || ""); const code = String(body.code || "").trim(); const name = String(body.name || "").trim();
    ...
    if (category === "CUSTOMER") {
      const taxId = String(masterMetadata.taxId || "").trim(); const email = String(masterMetadata.email || "").trim(); const paymentTerms = Number(masterMetadata.paymentTerms || 0);
    ...
    const id = crypto.randomUUID();
    try {
      await db.insert(masterData).values({ id, category, code, name, description: String(body.description || ""), status: "Active", metadata: JSON.stringify(masterMetadata), createdBy: user.email, createdAt: now, updatedAt: now });
    } catch { return jsonError("รหัส Master ซ้ำหรือข้อมูลไม่ถูกต้อง", 409); }
    await db.insert(auditLogs).values({ recordId: id, action: "CREATE_MASTER", actorEmail: user.email, details: `${category}: ${code} ${name}`, createdAt: now });
    return NextResponse.json({ ok: true, id });
  }
  ...
  if (body.action === "update_master") {
    ...
    const code = String(body.code || "").trim(); const name = String(body.name || "").trim(); const description = String(body.description || "").trim();
    ...
    if (item.category === "CUSTOMER") {
      const taxId = String(metadata.taxId || "").trim(); const email = String(metadata.email || "").trim(); const paymentTerms = Number(metadata.paymentTerms || 0);
      if (taxId && !/^\d{13}$/.test(taxId)) return jsonError("เลขประจำตัวผู้เสียภาษีต้องมี 13 หลัก", 422);
      if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return jsonError("รูปแบบอีเมลลูกค้าไม่ถูกต้อง", 422);
      if (!Number.isFinite(paymentTerms) || paymentTerms < 0 || paymentTerms > 365) return jsonError("จำนวนวันเครดิตไม่ถูกต้อง", 422);
    }
    if (item.category === "USER") {
      ...
      let currentRole = "Viewer"; try { currentRole = String((JSON.parse(item.metadata) as { role?: string }).role || "Viewer"); } catch {}
      if (currentRole === "Admin" && nextRole !== "Admin") {
        const users = await db.select().from(masterData).where(eq(masterData.category, "USER"));
        const activeAdmins = users.filter((candidate) => { try { return candidate.status === "Active" && (JSON.parse(candidate.metadata) as { role?: string }).role === "Admin"; } catch { return false; } });
        if (activeAdmins.length <= 1) return jsonError("ระบบต้องมี Admin ที่ใช้งานอยู่อย่างน้อย 1 คน", 409);
      }
    }
    try {
      await db.update(masterData).set({ code, name, description, metadata: JSON.stringify(metadata), updatedAt: now }).where(eq(masterData.id, id));
    } catch { return jsonError("รหัส Master ซ้ำหรือข้อมูลไม่ถูกต้อง", 409); }
    await db.insert(auditLogs).values({ recordId: id, action: "UPDATE_MASTER", actorEmail: user.email, details: `${item.category}: ${item.code} → ${code}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }

  if (body.action === "toggle_master") {
    const masterId = String(body.id || ""); const [item] = await db.select().from(masterData).where(eq(masterData.id, masterId));
    if (!item) return jsonError("ไม่พบ Master Data", 404);
    if (item.category === "USER" ? !hasPermission(access, "manage_users") : !hasPermission(access, "manage_master")) return deny();
    if (item.category === "USER" && item.status === "Active") {
      ...
      let role = "Viewer"; try { role = String((JSON.parse(item.metadata) as { role?: string }).role || "Viewer"); } catch {}
      if (role === "Admin") {
        const users = await db.select().from(masterData).where(eq(masterData.category, "USER"));
        const activeAdmins = users.filter((candidate) => { try { return candidate.status === "Active" && (JSON.parse(candidate.metadata) as { role?: string }).role === "Admin"; } catch { return false; } });
        if (activeAdmins.length <= 1) return jsonError("ระบบต้องมี Admin ที่ใช้งานอยู่อย่างน้อย 1 คน", 409);
    ...
    await db.insert(auditLogs).values({ recordId: masterId, action: "TOGGLE_MASTER", actorEmail: user.email, details: `${item.code}: ${item.status} → ${status}`, createdAt: now });
    return NextResponse.json({ ok: true });
  }
  ...
  const auditActions: Record<WorkflowAction, string> = { approve: "APPROVE", reject: "REJECT", post: "POST", issue: "ISSUE_DOCUMENT", reconcile: "RECONCILE", retry: "RETRY_INTEGRATION", complete: "COMPLETE_TASK", void: "VOID", set_status: "STATUS_CHANGE" };
  const auditAction = auditActions[body.action as WorkflowAction];
  await db.update(financialRecords).set({ status, approver: ["approve", "reject"].includes(body.action) ? user.email : record.approver, postedAt: body.action === "post" ? now : record.postedAt, updatedAt: now }).where(eq(financialRecords.id, id));
  await db.insert(auditLogs).values({ recordId: id, action: auditAction, actorEmail: user.email, details: `${record.documentNo}: ${record.status} → ${status}`, createdAt: now });
  return NextResponse.json({ ok: true });
}

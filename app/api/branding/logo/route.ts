...
import { getHostedLogo, getHostedLogoKey, resetHostedLogo, saveHostedLogo, usesHostedPreviewStorage } from "@/lib/hosted-branding";

export const dynamic = "force-dynamic";
const LOGO_SETTING = "brand_logo_key";

async function currentLogoKey() {
  if (usesHostedPreviewStorage()) return getHostedLogoKey();
...
export async function GET(request: Request) {
  ...
  if (!objectKey) return NextResponse.redirect(new URL("/account360-logo.png", request.url));
  if (objectKey.includes("-365_")) return NextResponse.redirect(new URL("/account360-logo.png", request.url));
  const hostedObject = usesHostedPreviewStorage() ? await getHostedLogo(objectKey) : null;
  const object = hostedObject ? new Uint8Array(await hostedObject.arrayBuffer()) : await getObject(objectKey);
  if (!object) return NextResponse.redirect(new URL("/account360-logo.png", request.url));
  const contentType = hostedObject?.httpMetadata?.contentType || (objectKey.toLowerCase().endsWith(".jpg") || objectKey.toLowerCase().endsWith(".jpeg") ? "image/jpeg" : "image/png");
...
export async function POST(request: Request) {
  ...
  if (usesHostedPreviewStorage()) {
    await saveHostedLogo(file, objectKey, user.email);
...
export async function DELETE() {
  ...
  if (!user) return NextResponse.json({ error: "กรุณาเข้าสู่ระบบ" }, { status: 401 });
  if (usesHostedPreviewStorage()) {
    await resetHostedLogo(user.email);

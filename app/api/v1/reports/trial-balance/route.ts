...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(await getTrialBalance(parsed.data.tenantId, parsed.data.companyId, parsed.data.periodId), { headers: { "cache-control": "private, no-store" } });
  } catch (error) {
    const code = error instanceof Error ? error.message : "TRIAL_BALANCE_FAILED";

...
export default function KCAccountApp({ initialUser, signOutHref }: { initialUser: ChatGPTUser; signOutHref: string }) {
  ...
  return (
    <LanguageContext.Provider value={language}><SidebarProvider className="app-shell" style={appTheme} data-theme={preferences.theme} data-density={preferences.tableDensity} data-page-width={preferences.pageWidth}>
      <Sidebar className="kc-sidebar" collapsible="offcanvas"><div className="kc-sidebar-shell"><KCNavigation active={active} data={data} selectPage={selectPage} /></div></Sidebar>
      <SidebarInset className="workspace">
        <header className="topbar"><div className="topbar-leading"><SidebarTrigger className="kc-sidebar-trigger" aria-label={language === "th" ? "เปิดหรือปิดแถบเมนู" : "Toggle navigation"} title={language === "th" ? "เปิดหรือปิดแถบเมนู" : "Toggle navigation"} /><div className="period-pill"><CalendarCheck /> {language === "th" ? "งวด" : "Period"} {data?.settings.current_period || "2026-09"}{data?.settings.locked_period && <LockKeyhole />}</div><button type="button" className="global-command-trigger" onClick={() => setCommandOpen(true)}><Search /><span>{language === "th" ? "ค้นหาทั่วทั้งระบบ" : "Search everything"}</span><kbd>Ctrl K</kbd></button></div><div className="top-actions"><LanguageMenu language={language} onChange={changeLanguage} /><button className="icon-button" onClick={() => setAuditOpen(true)} title="Audit log"><History /></button><div className="user-avatar">{(initialUser.fullName || initialUser.email).slice(0, 1).toUpperCase()}</div><div className="user-copy"><strong>{initialUser.fullName || "Finance User"}</strong><span>{data?.access.role || "Loading"} · {initialUser.email}</span></div><a className="icon-button" href={signOutHref} title={language === "th" ? "ออกจากระบบ" : "Sign out"} aria-label={language === "th" ? "ออกจากระบบ" : "Sign out"}><LogOut /></a></div></header>
...
function UserPermissionWorkspace({ data, working, mutate, canManage }: { data: AppData; working: boolean; mutate: (p: Record<string, unknown>, s: string) => Promise<boolean>; canManage: boolean }) {
  ...
  const displayNameFor = (item: MasterItem) => {
    ...
    return item.name.split("@")[0].split(/[._-]/).filter(Boolean).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ") || item.name;
  };
  ...
  const visibleUsers = users.filter((item) => {
    const meta = safeMeta(item.metadata); const policy = policyFor(meta.role);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const ok = await mutate({
      ...
      category: "USER",
      code: form.get("code"),
      name: String(form.get("email") || "").trim().toLowerCase(),
  ...
  return <div className="user-permission-workspace">
    <div className="user-summary-grid">
      <article><span className="user-summary-icon active"><Users /></span><div><small>{tr("ผู้ใช้ Active", "Active users")}</small><strong>{activeUsers}</strong></div></article>
      <article><span className="user-summary-icon pending"><History /></span><div><small>{tr("รอยืนยัน", "Pending")}</small><strong>{pendingUsers}</strong></div></article>
      <article><span className="user-summary-icon roles"><ShieldCheck /></span><div><small>{tr("Roles ทั้งหมด", "Total roles")}</small><strong>{DEFAULT_POLICY_TEMPLATES.length}</strong></div></article>
      <article><span className="user-summary-icon policies"><KeyRound /></span><div><small>{tr("Approval Policies", "Approval policies")}</small><strong>{approvalPolicies.length}</strong></div></article>
    </div>

    <section className="panel user-management-panel">
      <nav className="user-workspace-tabs" aria-label={tr("ส่วนจัดการผู้ใช้และสิทธิ์", "User and permission sections")}>{tabs.map((item) => <button key={item.id} type="button" className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{language === "th" ? item.th : item.en}</button>)}</nav>

      {tab === "users" && <>
        <div className="user-management-head"><div><h2>{tr("จัดการผู้ใช้", "User Management")}</h2><p>{tr("บัญชีผู้ใช้และสิทธิ์สร้างโดย Super Admin เท่านั้น", "User accounts and permissions are created by Super Admin only.")}</p></div><div className="user-management-actions"><label className="user-search"><Search /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tr("ค้นหาชื่อ อีเมล หรือทีม", "Search name, email, or team")} aria-label={tr("ค้นหาผู้ใช้", "Search users")} /></label>{canManage && <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus />{tr("สร้าง User", "Create User")}</Button>}</div></div>
        <div className="table-scroll"><Table className="user-management-table"><TableHeader><TableRow><TableHead>{tr("ผู้ใช้", "User")}</TableHead><TableHead>{tr("ทีม / แผนก", "Team / Department")}</TableHead><TableHead>Role</TableHead><TableHead>Data Scope</TableHead><TableHead>{tr("สถานะ", "Status")}</TableHead><TableHead className="action-cell">{tr("จัดการ", "Manage")}</TableHead></TableRow></TableHeader><TableBody>{visibleUsers.map((item) => { const meta = safeMeta(item.metadata); const policy = policyFor(meta.role); return <TableRow key={item.id}><TableCell><strong className="user-display-name">{displayNameFor(item)}</strong><small className="cell-sub">{item.name}</small><small className="user-ready">{item.status === "Active" ? tr("บัญชี Sign In พร้อมใช้", "Sign-in account ready") : tr("รอเปิดใช้งานบัญชี", "Account activation pending")}</small></TableCell><TableCell>{String(meta.department || policy?.department || item.description.split("·")[0] || "—")}</TableCell><TableCell><Badge variant="outline" className="user-role-badge">{policy?.name || String(meta.role || "—")}</Badge></TableCell><TableCell>{scopeLabel(meta.scope)}</TableCell><TableCell><StatusBadge status={item.status} /></TableCell><TableCell className="action-cell">{canManage && <div className="user-row-actions"><Button size="sm" variant="outline" disabled={working} onClick={() => { setEditing(item); setOpen(true); }}>{tr("แก้ไข", "Edit")}</Button><Button size="sm" variant="ghost" className="suspend-user" disabled={working} onClick={() => mutate({ action: "toggle_master", id: item.id }, tr("อัปเดตสถานะผู้ใช้แล้ว", "User status updated"))}>{item.status === "Active" ? tr("ระงับ", "Suspend") : tr("เปิดใช้", "Activate")}</Button></div>}</TableCell></TableRow>; })}{!visibleUsers.length && <TableRow><TableCell colSpan={6}><div className="empty-row"><Search />{tr("ไม่พบผู้ใช้ที่ตรงกับการค้นหา", "No users match your search.")}</div></TableCell></TableRow>}</TableBody></Table></div>
      </>}

      {tab === "roles" && <PolicyMatrix preview={data.settings.preview_mode === "true"} canManage={canManage} />}

      {tab === "dashboard" && <div className="user-tab-content"><div className="user-tab-heading"><div><h2>{tr("การมองเห็น Dashboard", "Dashboard Visibility")}</h2><p>{tr("กำหนดมุมมองเริ่มต้นตาม Role โดยอ้างอิงโมดูลที่ Policy อนุญาต", "Default dashboard visibility follows the modules allowed by each policy.")}</p></div><Badge variant="outline"><LayoutDashboard />{DEFAULT_POLICY_TEMPLATES.length} Roles</Badge></div><div className="table-scroll"><Table><TableHeader><TableRow><TableHead>Role</TableHead><TableHead>{tr("ฝ่าย", "Department")}</TableHead><TableHead>{tr("ส่วนที่มองเห็น", "Visible modules")}</TableHead><TableHead>{tr("สถานะ", "Status")}</TableHead></TableRow></TableHeader><TableBody>{DEFAULT_POLICY_TEMPLATES.map((policy) => <TableRow key={policy.key}><TableCell><strong>{policy.name}</strong></TableCell><TableCell>{policy.department}</TableCell><TableCell>{policy.modules.join(", ")}</TableCell><TableCell><StatusBadge status="Active" /></TableCell></TableRow>)}</TableBody></Table></div></div>}

      {tab === "approval" && <div className="user-tab-content"><div className="user-tab-heading"><div><h2>{tr("Approval Workflow", "Approval Workflow")}</h2><p>{tr("Role ที่มีสิทธิ์อนุมัติเอกสารและขอบเขตโมดูลที่รับผิดชอบ", "Roles allowed to approve documents and their responsible modules.")}</p></div><Badge variant="outline"><KeyRound />{approvalPolicies.length} Policies</Badge></div><div className="table-scroll"><Table><TableHeader><TableRow><TableHead>Role</TableHead><TableHead>{tr("ฝ่าย", "Department")}</TableHead><TableHead>{tr("โมดูลอนุมัติ", "Approval modules")}</TableHead><TableHead>{tr("รายการรออนุมัติ", "Pending approvals")}</TableHead></TableRow></TableHeader><TableBody>{approvalPolicies.map((policy) => <TableRow key={policy.key}><TableCell><strong>{policy.name}</strong><small className="cell-sub">{policy.description}</small></TableCell><TableCell>{policy.department}</TableCell><TableCell>{policy.modules.join(", ")}</TableCell><TableCell><Badge variant="outline">{data.records.filter((record) => record.status === "Pending Approval" && (policy.modules.includes("ALL") || policy.modules.includes(record.module))).length}</Badge></TableCell></TableRow>)}</TableBody></Table></div></div>}

      {tab === "audit" && <div className="user-tab-content"><div className="user-tab-heading"><div><h2>{tr("บันทึกการเปลี่ยนแปลงผู้ใช้และสิทธิ์", "User and permission audit log")}</h2><p>{tr("ตรวจสอบผู้ดำเนินการ เวลา และรายละเอียดการเปลี่ยนแปลงย้อนหลัง", "Review who changed access, when it changed, and what was updated.")}</p></div><Badge variant="outline"><History />{data.audit.length} {tr("รายการ", "entries")}</Badge></div><div className="table-scroll"><Table><TableHeader><TableRow><TableHead>{tr("เวลา", "Time")}</TableHead><TableHead>{tr("การดำเนินการ", "Action")}</TableHead><TableHead>{tr("รายละเอียด", "Details")}</TableHead><TableHead>{tr("ผู้ดำเนินการ", "Actor")}</TableHead></TableRow></TableHeader><TableBody>{data.audit.slice(0, 20).map((log) => <TableRow key={log.id}><TableCell>{dateText(log.createdAt)}</TableCell><TableCell><strong>{log.action.replaceAll("_", " ")}</strong></TableCell><TableCell>{log.details}</TableCell><TableCell>{log.actorEmail}</TableCell></TableRow>)}</TableBody></Table></div></div>}
    </section>

    <Dialog open={open} onOpenChange={(value) => { setOpen(value); if (!value) setEditing(null); }}><DialogContent key={editing?.id || "new-user"} className="create-dialog"><form onSubmit={submit}><DialogHeader><DialogTitle>{editing ? tr("แก้ไขผู้ใช้", "Edit user") : tr("สร้างผู้ใช้", "Create user")}</DialogTitle><DialogDescription>{tr("กำหนด Role และ Data Scope ให้เหมาะกับหน้าที่ ระบบจะบันทึกการเปลี่ยนแปลงใน Audit Log", "Assign a role and data scope for the user's duties. Changes are recorded in the audit log.")}</DialogDescription></DialogHeader><div className="form-grid master-form"><label>{tr("รหัสผู้ใช้", "User code")}<Input name="code" required defaultValue={editing?.code || ""} placeholder="USR-001" /></label><label>{tr("อีเมล Sign In", "Sign-in email")}<Input name="email" type="email" required defaultValue={editing?.name || ""} /></label><label>{tr("ชื่อที่แสดง", "Display name")}<Input name="displayName" required defaultValue={editing ? displayNameFor(editing) : ""} /></label><label>{tr("ทีม / แผนก", "Team / Department")}<Input name="department" required defaultValue={editing ? String(safeMeta(editing.metadata).department || policyFor(safeMeta(editing.metadata).role)?.department || "") : ""} /></label><label>Role<select name="role" defaultValue={String(safeMeta(editing?.metadata || "{}").role || "INTERNAL_AUDITOR")}>{DEFAULT_POLICY_TEMPLATES.map((policy) => <option key={policy.key} value={policy.key}>{policy.name}</option>)}</select></label><label>Data Scope<select name="scope" defaultValue={String(safeMeta(editing?.metadata || "{}").scope || "All branches")}><option value="Own data">{tr("ข้อมูลตนเอง", "Own data")}</option><option value="Team">{tr("เฉพาะทีม", "Team")}</option><option value="All branches">{tr("ทุกสาขา", "All branches")}</option><option value="All company">{tr("ทั้งบริษัท", "All company")}</option></select></label><label className="wide">{tr("รายละเอียด", "Description")}<Input name="description" defaultValue={editing?.description || ""} placeholder={tr("หน้าที่หรือหมายเหตุเพิ่มเติม", "Responsibilities or additional notes")} /></label></div><DialogFooter><Button type="button" variant="outline" onClick={() => { setOpen(false); setEditing(null); }}>{tr("ยกเลิก", "Cancel")}</Button><Button type="submit" disabled={working}>{tr("บันทึกผู้ใช้", "Save user")}</Button></DialogFooter></form></DialogContent></Dialog>
  </div>;
}
...
function SettingsView({ data, working, mutate, canManage, preferences, savePreferences, onBrandLogoChange }: { data: AppData; working: boolean; mutate: (p: Record<string, unknown>, s: string) => Promise<boolean>; canManage: boolean; preferences: UserPreferences; savePreferences: (updates: Partial<UserPreferences>) => Promise<UserPreferences>; onBrandLogoChange: (key: string) => void }) {
  ...
  async function uploadLogo(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onBrandLogoChange(body.key); toast.success(tr("อัปโหลด Company Logo แล้ว", "Company logo uploaded"));
  ...
  async function resetLogo() {
    setLogoBusy(true);
    try { const response = await fetch("/api/branding/logo", { method: "DELETE" }); await readApiJson<{ ok: boolean }>(response); onBrandLogoChange(""); toast.success(tr("กลับไปใช้ Logo บริษัทมาตรฐานแล้ว", "Default company logo restored")); }
    ...
    <form onSubmit={save}>
      ...
      <div className="settings-layout">
        <div className="settings-main-column">
          ...
          <section className="settings-surface"><div className="settings-section-head"><span className="settings-section-icon"><FileImage /></span><div><h2>Company Logo</h2><p>{tr("ใช้กับเอกสารบัญชีและข้อมูลบริษัทเท่านั้น โดยไม่เปลี่ยน Application Logo", "Used only for accounting documents and company identity; it does not change the application logo.")}</p></div></div><div className="brand-logo-editor"><div className="brand-logo-preview"><Image src={logoSrc} width={2172} height={724} alt="Company Logo" unoptimized /></div><div><strong>{tr("อัปโหลด Logo ขององค์กร", "Upload organization logo")}</strong><p>{tr("แนะนำ PNG พื้นหลังโปร่งใส อัตราส่วนแนวนอน · สูงสุด 1 MB", "Recommended: transparent PNG, landscape ratio · maximum 1 MB")}</p><input ref={logoInputRef} className="visually-hidden" type="file" accept="image/png,image/jpeg" onChange={uploadLogo} disabled={!canManage || logoBusy} /><div className="brand-logo-actions"><Button type="button" size="sm" variant="outline" onClick={() => logoInputRef.current?.click()} disabled={!canManage || logoBusy}>{logoBusy ? <Loader2 className="spin" /> : <Upload />}{tr("เลือกไฟล์", "Choose file")}</Button><Button type="button" size="sm" variant="outline" onClick={resetLogo} disabled={!canManage || logoBusy}>{tr("ใช้ Logo บริษัทมาตรฐาน", "Use default company logo")}</Button></div></div></div></section>

...
import {
  Activity, ArrowDownLeft, ArrowUpRight, Banknote, BellRing, BookOpen, BrainCircuit, Building2, CalendarCheck,
  Check, ChevronDown, ChevronRight, CircleCheck, CircleDollarSign, ClipboardCheck, Download, FileBarChart,
  Copy, FileImage, FileText, Gauge, Globe2, History, KeyRound, Landmark, LayoutDashboard, Link2, Loader2, LockKeyhole,
  LogOut, Palette, Paperclip, Pencil, Plus, Printer, RefreshCw, RotateCcw, Save, Search, Settings, ShieldCheck, SlidersHorizontal,
...
type AppData = { user: ChatGPTUser; access: { role: string; permissions: string[] }; insights: FinanceInsight[]; records: RecordItem[]; audit: AuditItem[]; settings: Record<string, string>; documents: DocumentItem[]; masters: MasterItem[]; connectors: ConnectorItem[]; integrationEvents: IntegrationEventItem[] };
...
export default function KCAccountApp({ initialUser }: { initialUser: ChatGPTUser }) {
  ...
  async function mutateIntegration(payload: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      if (body.apiKey) setIssuedKey({ name: String(payload.system || "Connector"), key: body.apiKey });
  ...
  const changeLanguage = (value: string) => { if (value !== "th" && value !== "en") return; setLanguage(value); window.localStorage.setItem("kc-account-language", value); document.documentElement.lang = value; };
  ...
  return (
    <LanguageContext.Provider value={language}><SidebarProvider className="app-shell" style={appTheme}>
      ...
      <SidebarInset className="workspace">
        <header className="topbar"><div className="topbar-leading"><SidebarTrigger className="kc-sidebar-trigger" aria-label={language === "th" ? "เปิดหรือปิดแถบเมนู" : "Toggle navigation"} title={language === "th" ? "เปิดหรือปิดแถบเมนู" : "Toggle navigation"} /><div className="period-pill"><CalendarCheck /> {language === "th" ? "งวด" : "Period"} {data?.settings.current_period || "2026-09"}{data?.settings.locked_period && <LockKeyhole />}</div></div><div className="top-actions"><ToggleGroup type="single" value={language} onValueChange={changeLanguage} className="language-switch" aria-label={language === "th" ? "เลือกภาษา" : "Select language"}><ToggleGroupItem value="th" aria-label="ภาษาไทย">TH</ToggleGroupItem><ToggleGroupItem value="en" aria-label="English">EN</ToggleGroupItem></ToggleGroup><button className="icon-button" onClick={() => setAuditOpen(true)} title="Audit log"><History /></button><div className="user-avatar">{(initialUser.fullName || initialUser.email).slice(0, 1).toUpperCase()}</div><div className="user-copy"><strong>{initialUser.fullName || "Finance User"}</strong><span>{data?.access.role || "Loading"} · {initialUser.email}</span></div><a className="icon-button" href="/signout-with-chatgpt?return_to=%2F" title={language === "th" ? "ออกจากระบบ" : "Sign out"} aria-label={language === "th" ? "ออกจากระบบ" : "Sign out"}><LogOut /></a></div></header>
...
function Dashboard({ data, onNavigate }: { data: AppData; onNavigate: (id: string) => void }) {
  ...
  return <div className="dashboard-grid">
    ...
    <section className="panel alert-panel"><div className="panel-head"><div><p className="eyebrow">ACTION REQUIRED</p><h2>{tr("รายการที่ต้องดำเนินการ", "Items requiring action")}</h2></div><button className="text-link" onClick={() => onNavigate("approval")}>{tr("ดูทั้งหมด", "View all")} <ChevronRight /></button></div><div className="action-list">{pending.slice(0, 4).map((r) => <button key={r.id} onClick={() => onNavigate("approval")}><div className="list-icon"><ClipboardCheck /></div><div><strong>{r.documentNo}</strong><span>{r.description}</span></div><b>{money(r.amount)}</b><ChevronRight /></button>)}{!pending.length && <div className="all-clear"><Check />{tr("ไม่มีรายการรออนุมัติ", "No items awaiting approval")}</div>}</div></section>
...
function SettingsView({ data, working, mutate, canManage }: { data: AppData; working: boolean; mutate: (p: Record<string, unknown>, s: string) => Promise<boolean>; canManage: boolean }) {
  const { tr } = useLanguage();
